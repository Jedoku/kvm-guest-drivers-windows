...
BOOL CMemStat::Init()
{
    ...
    status = CoInitializeSecurity(
            NULL, -1, NULL, NULL, RPC_C_AUTHN_LEVEL_PKT, RPC_C_IMP_LEVEL_IMPERSONATE, NULL, EOAC_NONE, 0);
    ...
    status = CoCreateInstance(
            CLSID_WbemLocator, NULL, CLSCTX_INPROC_SERVER, IID_IWbemLocator, reinterpret_cast<void **>(&locator));
...
BOOL CMemStat::Update()
{
    ...
    if (enumerator == NULL || FAILED(enumerator->Next(WBEM_INFINITE, 1L, &memory, &retcnt)))
    {
    ...
    if (retcnt > 0)
    {
        status = memory->Get(L"PagesInputPerSec", 0, &var_val, NULL, NULL);
        ...
        status = memory->Get(L"PagesOutputPerSec", 0, &var_val, NULL, NULL);
        ...
        status = memory->Get(L"PageReadsPerSec", 0, &var_val, NULL, NULL);
        ...
        status = memory->Get(L"PageFaultsPerSec", 0, &var_val, NULL, NULL);
        ...
        status = memory->Get(L"CacheBytes", 0, &var_val, NULL, NULL);
        ...
        else if (GetSystemFileCacheSize(&minCacheSize, &maxCacheSize, &flags) && (flags & FILE_CACHE_MIN_HARD_ENABLE) &&
